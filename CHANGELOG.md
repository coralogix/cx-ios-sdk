...
- `SessionReplayInterface` gained a required `captureEvent(properties:completion:)`. The protocol wires the `SessionReplay` module to the main SDK, so apps consuming `SessionReplay.shared` are unaffected; a type conforming to it directly adds the member. There is no default implementation, because a conformer that silently never reports an outcome would leave every screenshot-carrying event unsent. Report the real outcome — whether a frame was captured — once it is known, and do not forward the synchronous overload's return value: it says the capture started, not that a frame exists, so an event stamped on that basis points at a screenshot that may never arrive. A conformer that genuinely cannot tell reports failure rather than success, so no event claims a screenshot it cannot back:
  ...
  func captureEvent(properties: [String: Any]?, completion: @escaping CaptureEventCompletion) {
      guard case .success = captureEvent(properties: properties) else {
          completion(.failure(.captureFailed))
          return
      }
      // Call completion(.success(())) only once the frame is known to have been captured,
...
- **A crash event is no longer held back from its force-flush.** A crash report is flushed immediately rather than waiting in the batch queue, because the process is usually about to die. Attaching the screenshot asynchronously meant the crash event was not yet in the batch when that flush ran, so the crash surfaced only on the next launch. A crash event now closes immediately and carries no screenshot rather than waiting for one it has no time to collect.
- **A tap capture now tells the Flutter plugin when the tap actually happened.** The value was read from a key every capture overwrites with its own wall clock, so it always described the moment the capture started and Dart measured a staleness of roughly zero — it could never decline to draw a tap it was too late to represent. It now comes from the touch's own clock, for taps and swipes alike.
- **An unanswered Flutter provider no longer holds events back.** Because a screenshot-carrying event now closes when its capture resolves, a wedged Dart isolate or a torn-down engine would have stopped error, log, tap and navigation events from being exported at all. A capture now gives up on the frame after one second and reports the event without a screenshot. A provider supplying its own bitmaps should answer within that second; a later answer is ignored rather than composited.
- **Returning a screenshot slot can no longer overwrite a frame that already shipped.** The counter was stepped back blindly, so a capture that dropped after a later one had already taken a slot made the next allocation reuse that slot — same page and segment index, so the upload overwrote the earlier frame and two events pointed at the same screenshot. A slot is now only returned when it is still the most recent one; otherwise it is left unused.
- **A Flutter screen no longer records as solid black.** The bitmap Dart supplies is already masked, but the capture painted Dart's mask rectangles over it a second time. Normally invisible — it repainted pixels that were already black — it turned the whole frame black as soon as a dialog opened, because Dart then reports every masked row behind the modal barrier and those rectangles cover the screen. The replay went black from that moment on while every upload reported success. Dart's rectangles are now reported for tap-marker suppression but never repainted, and they no longer decide whether a frame is a duplicate either — read that way, the same whole-screen over-report made every tap taken while a dialog was open look like a tap on masked content, and dropped its frame. A window holding a FlutterView with no bitmap to paste drops the capture when a bitmap provider was asked and did not answer. With no provider configured at all — an add-to-app host embedding a `FlutterViewController` without the plugin — the region is black-filled and the rest of the frame is kept, as before, since those pixels are not obtainable by any means.
- **A tap on a masked element no longer records a duplicate frame.** A tap capture skips deduplication so its marker is not lost, but a tap inside a masked region is deliberately never drawn, so that frame carried nothing the previous one did not. It is now deduplicated like any other.
- **A small change on screen is no longer mistaken for a duplicate frame.** Deduplication compared the average difference across the whole frame, so a change confined to one control — a checkbox toggling, a character typed, a badge counting up — was divided by the entire screen and fell under the threshold. Any edit smaller than about one percent of the screen was dropped, and the replay skipped straight past it. Frames are now compared the way the Android SDK compares them: a difference hash, a structural similarity score and a per-tile scan each vote, and all three must agree before a frame is discarded, so a change any one of them can see is kept. The tile scan reads colour rather than brightness, and any single changed tile keeps the frame — two points where this deliberately differs from Android, both only ever keeping more frames. Without the first, a change carried entirely in hue was invisible to every metric at once, because Rec.601 maps distinct hues onto the same brightness: a status dot or a validation field going red to green scored a perfect similarity match and was dropped. Without the second, a single changed tile out of 256 still counted as unchanged. Comparison also runs before compression now, so a discarded frame costs no encode.
- **A scroll or a swipe is no longer reported to the Flutter plugin as a tap.** The flag was derived from whether the capture carried a touch position, which is true of every interaction, so Dart was asked to hold a scroll for a committed frame and judge it against a tap age — for a gesture with no single moment to be late for.
- **A manual `captureEvent()` on an unchanged screen now reports its frame.** Identical frames are deduplicated, and a manual capture was silently subject to that too: the second call on a static screen produced no event and no log, with nothing in the void-returning API to say so. A capture the host requests directly — through `SessionReplay.shared.captureEvent` or `CoralogixRum.captureEvent()` — now bypasses deduplication, as a tap capture already did. A caller that reserves its own slot first with `getNextScreenshotLocationProperties()` is indistinguishable from the SDK's own periodic capture and is still deduplicated; set `is_manual` in the properties to opt out.
