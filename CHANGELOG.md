...
- **A Flutter screen no longer records as solid black.** When the capture decided there was no Flutter view to ask Dart about, but the render pass then found one in the window, it filled that window black and shipped the result as a real frame — a replay that went black the moment a dialog opened, with every upload reporting success. Such a capture is now dropped, which is the rule the Flutter path already followed when Dart answered with no frame: never a black fill, never the raw GPU surface.
