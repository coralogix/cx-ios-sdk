...
- **A manual `captureEvent()` on an unchanged screen now reports its frame.** Identical frames are deduplicated, and a manual capture was silently subject to that too: the second call on a static screen produced no event and no log, with nothing in the void-returning API to say so. Any capture the host requests directly — through `SessionReplay.shared.captureEvent`, `CoralogixRum.captureEvent()` or the plugin bridge — now bypasses deduplication, as a tap capture already did.
