...
class CoralogixUITestCase: XCTestCase {
    ...
    /// Fetches captured OTLP batches via the localhost endpoint and converts
    /// them into `[[String: Any]]` log entries that mirror the shape the old
    /// schema-validator path produced — i.e. each entry is
    /// `{ "text": { "cx_rum": { "interaction_context": {...}, ... } } }`.
    /// This lets existing `extractInteractionContext` / `hasInteractionEvent`
    /// helpers work unchanged.
    func inProcessLogEntries(timeout: TimeInterval? = nil) -> [[String: Any]]? {
        guard let batches = fetchCapturedBatches(timeout: timeout) else { return nil }
        var entries: [[String: Any]] = []
        for jsonString in batches {
            guard let data = jsonString.data(using: .utf8),
                  let root = try? JSONSerialization.jsonObject(with: data) as? [String: Any],
                  let resourceSpans = root["resourceSpans"] as? [[String: Any]] else { continue }
            for rs in resourceSpans {
                ...
                for ss in scopeSpans {
                    ...
                    for span in spans {
                        ...
                        if let attrs = span["attributes"] as? [[String: Any]] {
                            for kv in attrs {
                                guard let key = kv["key"] as? String,
                                      let valueObj = kv["value"] as? [String: Any] else { continue }
                                let path = key.split(separator: ".").map(String.init)
                                Self.setNested(&nested, path: path, value: Self.unwrapOtlpValue(valueObj))
                            }
                        }
                        entries.append(["text": nested])
                    }
                }
            }
        }
    ...
    private static func unwrapOtlpValue(_ obj: [String: Any]) -> Any {
        if let s = obj["stringValue"] as? String { return s }
        if let b = obj["boolValue"] as? Bool { return b }
        if let i = obj["intValue"] { return i }
        if let d = obj["doubleValue"] { return d }
        if let arr = obj["arrayValue"] as? [String: Any],
           let values = arr["values"] as? [[String: Any]] {
            return values.map(unwrapOtlpValue)
        }
        return ""
    }
