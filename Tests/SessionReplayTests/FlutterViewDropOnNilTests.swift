...
final class FlutterViewDropOnNilTests: XCTestCase {
    ...
    func testDeliveredBitmap_isComposited() throws {
        ...
        let frame = try XCTUnwrap(model.resolveFlutterFrame(freshBitmap: red, frameId: 1),
    ...
    func testNilDelivery_afterAGoodFrame_stillDropsInsteadOfReusingIt() throws {
        ...
        XCTAssertNil(model.resolveFlutterFrame(freshBitmap: nil, frameId: 3),
    ...
    func testOutOfOrderDelivery_isDropped() throws {
        ...
        XCTAssertNil(model.resolveFlutterFrame(freshBitmap: red, frameId: 3),
    ...
    func testRepeatedFrameId_isDropped() throws {
        let red = try solidBitmap(width: 2, height: 2, r: 255, g: 0, b: 0)
        _ = try XCTUnwrap(model.resolveFlutterFrame(freshBitmap: red, frameId: 7))

        XCTAssertNil(model.resolveFlutterFrame(freshBitmap: red, frameId: 7),
    ...
    func testNewerDelivery_isComposited() throws {
        ...
        let latest = try XCTUnwrap(model.resolveFlutterFrame(freshBitmap: green, frameId: 2),
                                   "A newer frameId must be accepted")
    ...
    func testSessionRotation_releasesTheAcceptedFrameIdWatermark() throws {
        let red = try solidBitmap(width: 2, height: 2, r: 255, g: 0, b: 0)
        _ = try XCTUnwrap(model.resolveFlutterFrame(freshBitmap: red, frameId: 9))
        ...
        XCTAssertNotNil(model.resolveFlutterFrame(freshBitmap: red, frameId: 2),
                        "A session rotation must reset the accepted-frameId watermark")
    }
